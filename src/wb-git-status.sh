...
if [[ $PROVIDER == "github.com" ]]; then
  ...
  PROVIDER_ICON="$RESET#[fg=${THEME[foreground]},bg=${THEME[ghbackground]}] "
  ...
  PROVIDER_ICON="$RESET#[fg=#fc6d26,bg=${THEME[ghbackground]}] "
...
if [[ $PR_COUNT -gt 0 ]]; then
  PR_STATUS="#[fg=${THEME[ghgreen]},bg=${THEME[ghbackground]},bold] ${RESET}${PR_COUNT} "
fi

if [[ $REVIEW_COUNT -gt 0 ]]; then
  REVIEW_STATUS="#[fg=${THEME[ghyellow]},bg=${THEME[ghbackground]},bold] ${RESET}${REVIEW_COUNT} "
fi

if [[ $ISSUE_COUNT -gt 0 ]]; then
  ISSUE_STATUS="#[fg=${THEME[ghgreen]},bg=${THEME[ghbackground]},bold] ${RESET}${ISSUE_COUNT} "
fi

if [[ $BUG_COUNT -gt 0 ]]; then
  BUG_STATUS="#[fg=${THEME[ghred]},bg=${THEME[ghbackground]},bold] ${RESET}${BUG_COUNT} "
fi

WB_STATUS="#[fg=${THEME[blue]},bg=${THEME[ghbackground]},bold] $RESET$PROVIDER_ICON $RESET$PR_STATUS$REVIEW_STATUS$ISSUE_STATUS$BUG_STATUS"
