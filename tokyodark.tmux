#!/usr/bin/env bash
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# title      tokyodark-tmux                                              +
# version    1.0.0                                                    +
# repository https://github.com/thederpykrafter/tokyodark-tmux           +
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
default_window_id_style="dfsquare"
default_pane_id_style="hsquare"
default_zoom_id_style="digital"

default_terminal_icon=""
default_active_terminal_icon=""

window_id_style="$(echo "$TMUX_VARS" | grep '@tokyodark-tmux_window_id_style' | cut -d" " -f2)"
pane_id_style="$(echo "$TMUX_VARS" | grep '@tokyodark-tmux_pane_id_style' | cut -d" " -f2)"
zoom_id_style="$(echo "$TMUX_VARS" | grep '@tokyodark-tmux_zoom_id_style' | cut -d" " -f2)"
terminal_icon="$(echo "$TMUX_VARS" | grep '@tokyodark-tmux_terminal_icon' | cut -d" " -f2)"
active_terminal_icon="$(echo "$TMUX_VARS" | grep '@tokyodark-tmux_active_terminal_icon' | cut -d" " -f2)"
window_tidy="$(echo "$TMUX_VARS" | grep '@tokyodark-tmux_window_tidy_icons' | cut -d" " -f2)"
